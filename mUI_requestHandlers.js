...
function bookReserve(response, postData, sessionData) {
    ...
    sql.connect(config).then(function () {
        ...
        switch (firstChar) {
            case 'S':
                queryOne = "SELECT count(*) as Count FROM Reservation WHERE Student_ID=@userID AND Active_Void_Status = 1";
                queryTwo = "SELECT count(*) as Count FROM Transactions WHERE StudentID=@userID AND Active_Void_Status = 1";
                break;
            case 'G':
                queryOne = "SELECT count(*) as Count FROM Reservation WHERE Guest_ID=@userID AND Active_Void_Status = 1";
                queryTwo = "SELECT count(*) as Count FROM Transactions WHERE Guest_ID=@userID AND Active_Void_Status = 1";
                break;
            case 'F':
                queryOne = "SELECT count(*) as Count FROM Reservation WHERE Faculty_ID=@userID AND Active_Void_Status = 1";
                queryTwo = "SELECT count(*) as Count FROM Transactions WHERE Faculty_ID=@userID AND Active_Void_Status = 1";
        }

        // if (await canReserveBook(response, sessionData) === true) {
        //     console.log("----------------------book limit reached!!-----------------------")
        //     response.write("Book borrow limit reached.");
        //     response.end();
        //     return;
        // } else {
        //     console.log("Else can reserve book")
        // }
...
function electronicReserve(response, postData, sessionData) {
    ...
    sql.connect(config).then(function () {
        ...
        req.query("SELECT Available FROM Electronics WHERE Serial_No=@serialno").then(function (recordset) {
            if (recordset.recordsets[0].length > 0) {
                var available = recordset.recordsets[0][0].Available;
                queryStr = "INSERT INTO Reservation (Creation_Date, Expiration_Date, Created_By, Updated_By, Faculty_ID, Electronics_ID, holdPosition, Active_Void_Status) VALUES (getdate(), getdate(), @userId, @userId, @userId, @serialno, 0, 1)";
                available = 0;
                queryAvail = "UPDATE Electronics SET Available = '" + 0 + "' WHERE Serial_No = '" + SerialNo + "';";

                req.query(queryStr).then(function (recordset) {
                    console.log("Electronic Reservation for " + SerialNo + " added");
                    req.query(queryAvail).then(function (recordset) {
                        console.log("Electronic Serial No: " + SerialNo + " not available anymore")
                        response.write("Reservation successful");
                        response.end();
                    })
                });
